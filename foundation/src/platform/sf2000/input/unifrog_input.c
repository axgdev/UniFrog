...
#define INPUT_SF2000_FALLBACK_PROBE_DIV 4u
#define INPUT_SF2000_FALLBACK_CONFIRM_POLLS 3u
#define INPUT_SF2000_FALLBACK_MAX_BUTTONS 4u
...
static unsigned sf2000_fallback_probe_phase;
static uint32_t sf2000_fallback_candidate_raw;
static uint32_t sf2000_fallback_candidate_norm;
static unsigned sf2000_fallback_candidate_count;
...
{
   if (unifrog_device_board() == UNIFROG_DEVICE_BOARD_GB300)
   ...
   uint32_t raw, uint32_t normalized)
{
   if (profile != local_input_profile) {
      ...
      unifrog_device_note_input_profile(profile == LOCAL_INPUT_STOCK_BITS,
         reason);
      unifrog_log("unifrog input local_profile=%s board=%s panel=%s lcd_id=0x%06lx reason=%s raw=0x%08lx norm=0x%08lx\n",
         local_input_profile_name(profile),
         unifrog_device_board_name(unifrog_device_board()),
         unifrog_device_panel_name(unifrog_device_panel()),
         unifrog_device_lcd_panel_id(), reason ? reason : "",
...
static int sf2000_input_candidate_valid(uint32_t raw, uint32_t normalized)
{
   unsigned pressed;

   normalized &= INPUT_VALID_BUTTON_MASK;
   raw &= INPUT_VALID_BUTTON_MASK;
   if (!normalized || raw == INPUT_VALID_BUTTON_MASK)
      return 0;
   if (raw != normalized)
      return 0;
   pressed = input_popcount(normalized);
   if (pressed == 0 || pressed > INPUT_SF2000_FALLBACK_MAX_BUTTONS)
      return 0;
   return 1;
}
...
static int probe_sf2000_input_fallback(uint32_t *raw_out,
   uint32_t *normalized_out)
{
   uint32_t raw;
   uint32_t normalized;
   ...
   if (detect_local_input_profile() != LOCAL_INPUT_STOCK_BITS)
      return 0;
   if ((sf2000_fallback_probe_phase++ %
       INPUT_SF2000_FALLBACK_PROBE_DIV) != 0)
      return 0;

   raw = scan_sf2000_local_raw();
   normalized = normalize_local_raw_for_profile(raw, LOCAL_INPUT_SF2000);
   if (sf2000_input_candidate_valid(raw, normalized)) {
      if (raw == sf2000_fallback_candidate_raw &&
          normalized == sf2000_fallback_candidate_norm)
         sf2000_fallback_candidate_count++;
      else {
         sf2000_fallback_candidate_raw = raw;
         sf2000_fallback_candidate_norm = normalized;
         sf2000_fallback_candidate_count = 1;
      }
      if (sf2000_fallback_candidate_count >=
          INPUT_SF2000_FALLBACK_CONFIRM_POLLS) {
         set_local_input_profile(LOCAL_INPUT_SF2000, "sf2000_bus", raw,
            normalized);
         if (raw_out)
            *raw_out = raw;
         if (normalized_out)
      ...
      sf2000_fallback_candidate_raw = 0;
      sf2000_fallback_candidate_norm = 0;
      sf2000_fallback_candidate_count = 0;
   }
   ...
   uint32_t *normalized_out)
{
   ...
   if (detect_local_input_profile() == LOCAL_INPUT_STOCK_BITS) {
      ...
      normalized_mask = normalize_local_raw_for_profile(raw_mask,
         LOCAL_INPUT_STOCK_BITS);
      /*
       * A swapped SF2000 can report a GB300 LCD panel even though the keypad
       * and audio gate are still SF2000.  Probe the SF2000 shifter when the
       * GB300-selected path is not producing a usable local button.
       */
      if (!normalized_mask || raw_mask == 0xffffu ||
          raw_mask == 0xffffffffu)
         (void)probe_sf2000_input_fallback(&raw_mask, &normalized_mask);
   } else {
