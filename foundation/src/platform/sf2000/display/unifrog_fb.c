...
static unsigned requested_buffers_from_flags(unsigned flags)
{
...
static void apply_requested_buffers(struct fb_var_screeninfo *var, unsigned flags)
{
   unsigned requested = requested_buffers_from_flags(flags);

   if (requested && var->yres)
      var->yres_virtual = var->yres * requested;
}
...
static int put_format_var(int fd, struct fb_var_screeninfo *var, unsigned flags)
{
   ...
   apply_requested_buffers(var, flags);
...
static int fb_var_has_requested_buffers(const struct fb_var_screeninfo *var,
   unsigned flags)
{
   unsigned requested = requested_buffers_from_flags(flags);
   ...
   if (!requested)
      return 1;
   return var && var->yres && var->yres_virtual >= var->yres * requested;
}

static void update_fb_info(struct unifrog_fb *fb,
...
int unifrog_fb_open(struct unifrog_fb *fb, unsigned flags)
{
   ...
   if (!(flags & UNIFROG_FB_OPEN_PRESERVE) ||
       ...
       !fb_var_has_requested_buffers(&var, flags)) {
      put_var = 1;
...
int unifrog_fb_set_buffer_count(struct unifrog_fb *fb, unsigned buffers)
{
   ...
   if (!fb || fb->fd < 0 || buffers == 0 || buffers > 4)
      return -1;
   ...
   if (ioctl(fb->fd, FBIOGET_FSCREENINFO, &fix) != 0 ||
       get_var(fb->fd, &var) != 0)
      return -1;
   if (var.yres == 0 || var.yres_virtual / var.yres < buffers)
      return -1;
   old_smem_len = fb->smem_len;
   if (fix.smem_len != old_smem_len) {
      mapped = mmap(NULL, fix.smem_len, PROT_READ | PROT_WRITE, MAP_SHARED,
         fb->fd, 0);
      if (mapped == MAP_FAILED)
         return -1;
      if (fb->pixels)
         munmap(fb->pixels, old_smem_len);
      fb->pixels = mapped;
   }
   update_fb_info(fb, &fix, &var);
   remap_ms = unifrog_perf_time_ms();
   unifrog_log("unifrog fb buffers old=%u requested=%u ret=0 "
      "total_ms=%lu get_ms=%lu put_ms=%lu remap_ms=%lu current=%u "
      "buffers=%u max=%u smem=%lu yvirt=%u\n",
      old_buffers, buffers, (unsigned long)(remap_ms - start_ms),
      (unsigned long)(get_ms - start_ms),
      (unsigned long)(put_ms - get_ms),
      (unsigned long)(remap_ms - put_ms), fb->current_buffer,
      fb->buffer_count, fb->max_buffers, (unsigned long)fb->smem_len,
      var.yres_virtual);
   return 0;
}
...
int unifrog_fb_pan(struct unifrog_fb *fb, unsigned buffer_index)
{
   ...
   if (!fb || fb->fd < 0 || buffer_index >= fb->buffer_count)
      return -1;
   /*
    * HCRTOS FBIOPAN_DISPLAY consumes only xoffset, yoffset, and vmode, then
    * validates them against the driver's stored mode. Avoid fetching that
    * unchanged stored mode before every emulator frame.
    */
   memset(&var, 0, sizeof(var));
   var.xoffset = 0;
