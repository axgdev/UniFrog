   ...
   size_t alignment, void **out_ptr)
{
   ...
   if (out_ptr)
      *out_ptr = NULL;
   if (!UNIFROG_ABI_HAS_MEMBER(abi, application_memory_reserve_top) ||
       !abi->application_memory_reserve_top)
      return -1;
   return abi->application_memory_reserve_top(bytes, alignment, out_ptr);
}
...
void unifrog_abi_application_memory_release_top(void *ptr)
{
   ...
   if (UNIFROG_ABI_HAS_MEMBER(abi, application_memory_release_top) &&
       abi->application_memory_release_top)
      abi->application_memory_release_top(ptr);
}
...
int chmod(const char *path, mode_t mode)
{
   ...
   (void)mode;
   /* FAT has no POSIX permission bits. Preserve the useful existence check
    * while treating permission changes as a successful no-op. */
   return stat(path, &st);
}

char *getcwd(char *buf, size_t size)
{
   static const char root[] = UNIFROG_SD_ROOT;
...
DIR *opendir(const char *path)
{
   ...
   if (!abi || !abi->opendir)
      return NULL;
   return abi->opendir(module_map_path(path, mapped, sizeof(mapped)));
}
...
struct dirent *readdir(DIR *dir)
{
   ...
   if (!abi || !abi->readdir || !dir)
      return NULL;
   return abi->readdir(dir);
}
...
int closedir(DIR *dir)
{
   ...
   if (!abi || !abi->closedir || !dir)
      return -1;
   return abi->closedir(dir);
}
...
/* Newlib implements clock() through times(), which is a kernel-only symbol in
 * HCRTOS. Core modules use the ABI-backed monotonic clock directly instead. */
clock_t clock(void)
{
