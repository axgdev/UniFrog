...
{
   unsigned target = fb->max_buffers >= 3 ? 3 : 2;

   if (preserve_logo && fb->buffer_count >= target)
      return;
   if (unifrog_fb_set_buffer_count(fb, target) != 0 &&
       unifrog_fb_set_buffer_count(fb, 2) != 0)
...
int unifrog_ui_open(struct unifrog_ui *ui, int preserve_logo)
{
   ...
   flags |= UNIFROG_FB_OPEN_BUFFERS_3;
   ...
   int max_chars, const char *text, uint16_t color, int scale)
{
   ...
   unifrog_text_utf8_copy_chars(clipped, sizeof(clipped), text,
      (size_t)(max_chars >= 3 ? max_chars - 3 : max_chars));
   if (max_chars >= 3) {
      size_t used = strlen(clipped);

      if (used + 3u < sizeof(clipped))
         memcpy(clipped + used, "...", 4u);
   }
   unifrog_ui_text(ui, x, y, clipped, color, scale);
}

int unifrog_ui_text_marquee(struct unifrog_ui *ui, int x, int y,
   int max_chars, const char *text, uint16_t color, int scale,
   uint32_t elapsed_ms)
{
   char visible[96];
   ...
   if (max_chars < 0)
      max_chars = 0;
   active = unifrog_text_marquee(visible, sizeof(visible), text,
      (size_t)max_chars, elapsed_ms);
   unifrog_ui_text(ui, x, y, visible, color, scale);
   return active;
}
