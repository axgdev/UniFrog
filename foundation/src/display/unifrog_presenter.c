...
static struct unifrog_ge_rect presenter_dst_rect(
   struct unifrog_presenter *presenter, unsigned src_w, unsigned src_h)
{
   struct unifrog_ge_rect rect;

   /*
    * Aspect-fit geometry normally stays constant for an entire core session.
    * Cache it because the cold calculation uses 64-bit division, which is
    * disproportionately expensive on the MIPS32r1 target.
    */
   if (presenter->last_src_w == src_w &&
       presenter->last_src_h == src_h &&
       presenter->last_fb_w == presenter->fb.width &&
       presenter->last_fb_h == presenter->fb.height &&
       presenter->last_flags == presenter->flags &&
       presenter->last_dst_w > 0 && presenter->last_dst_h > 0) {
      rect.x = presenter->last_dst_x;
      rect.y = presenter->last_dst_y;
      rect.w = presenter->last_dst_w;
      rect.h = presenter->last_dst_h;
      return rect;
   }

   rect = scaled_rect(src_w, src_h, presenter->fb.width,
      presenter->fb.height, presenter->flags);
   presenter->last_src_w = src_w;
   presenter->last_src_h = src_h;
   presenter->last_fb_w = presenter->fb.width;
   presenter->last_fb_h = presenter->fb.height;
   presenter->last_flags = presenter->flags;
...
{
   unsigned fb_flags = UNIFROG_FB_OPEN_DEFAULT;
   ...
   if (buffers == 0)
      buffers = 2;
   if (buffers >= 3)
      fb_flags |= UNIFROG_FB_OPEN_BUFFERS_3;
   else if (buffers >= 2)
      fb_flags |= UNIFROG_FB_OPEN_BUFFERS_2;

   if (unifrog_fb_open(&presenter->fb, fb_flags) != 0)
      goto fail;
   ...
   enum unifrog_ge_format format)
{
   ...
   dst_rect = presenter_dst_rect(presenter, width, height);
   ...
   ge_count = unifrog_perf_elapsed(ge_start, unifrog_perf_count());
   presenter->present_ge_count += (uint64_t)ge_count;
   if (ret != 0)
      return ret;
   ...
   ret = unifrog_ge_sync(&presenter->ge);
   sync_count = unifrog_perf_elapsed(sync_start, unifrog_perf_count());
   presenter->present_sync_count += (uint64_t)sync_count;
   if (ret != 0)
      return ret;
   if (presenter->flags & UNIFROG_PRESENT_VSYNC) {
   ...
   pan_count = unifrog_perf_elapsed(pan_start, unifrog_perf_count());
   presenter->present_pan_count += (uint64_t)pan_count;
   if (ret == 0)
   ...
   presenter->last_present_count = total_count;
   presenter->last_ge_count = ge_count;
   presenter->last_sync_count = sync_count;
   presenter->last_vsync_count = vsync_count;
   presenter->last_pan_count = pan_count;
