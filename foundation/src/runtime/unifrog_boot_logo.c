...
static void blit_logo_rle(struct unifrog_surface *surface,
   unsigned x0, unsigned y0)
{
   const unsigned logo_pixels = BOOT_LOGO_WIDTH * BOOT_LOGO_HEIGHT;
   unsigned pos = 0;

   for (unsigned i = 0; i + 1 < UNIFROG_BOOT_LOGO_RLE_WORDS; i += 2u) {
      uint16_t color = unifrog_boot_logo_rle[i];
      unsigned count = unifrog_boot_logo_rle[i + 1u];

      while (count != 0 && pos < logo_pixels) {
         unsigned x = pos % BOOT_LOGO_WIDTH;
         unsigned y = pos / BOOT_LOGO_WIDTH;
         unsigned run = BOOT_LOGO_WIDTH - x;
         uint16_t *dst;
