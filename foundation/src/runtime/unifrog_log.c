...
#define MEMLOG_PERSIST_BASE 0x87e00000u
#define MEMLOG_PERSIST_BYTES (FASTBOOT_CHUNK_ADDR - MEMLOG_PERSIST_BASE)
...
static size_t memlog_auto_flush_bytes = UNIFROG_LOG_AUTO_FLUSH_BYTES;
static int memlog_flush_every;
...
static void memlog_retry_delay_us(useconds_t us)
{
   uint32_t start;
   uint32_t ticks;
   ...
   start = mips_getcount();
   ticks = (uint32_t)us * (CONFIG_CPU_CLOCK_HZ / 2000000u);
   while ((uint32_t)(mips_getcount() - start) <= ticks)
      ;
}
...
static void memlog_activity_set(uint32_t phase, uint32_t pending,
   uint32_t stage)
{
   const char *path = memlog_last_path ? memlog_last_path : "log";

   unifrog_exception_activity_set(phase,
      unifrog_exception_activity_hash(path), pending, stage);
}
...
static int memlog_open_retry(int flags)
{
   ...
   for (unsigned i = 0; i < MEMLOG_OPEN_RETRIES; i++) {
      ...
      if (fd >= 0)
         return fd;
      memlog_retry_delay_us(MEMLOG_OPEN_RETRY_US);
   }
   return -1;
}
...
static int memlog_flush_recovery(void)
{
   ...
   pending = memlog_len;
   memlog_activity_set(UNIFROG_ACTIVITY_PHASE_LOG_RECOVERY,
      (uint32_t)pending, 1);
...
static int memlog_vappend(enum unifrog_log_level level, const char *component,
   const char *fmt, va_list ap)
{
   ...
   out_len = snprintf(out, sizeof(out),
      "UFLOG ts_ms=%lu dt_ms=%lu cyc=%08lx seq=%lu level=%s component=%s msg=%.*s%s",
      (unsigned long)now_ms, (unsigned long)(now_ms - memlog_last_ms),
      (unsigned long)count, (unsigned long)++memlog_sequence,
      unifrog_log_level_name(level), component, len, line,
      line[len - 1] == '\n' ? "" : "\n");
...
int unifrog_log(const char *fmt, ...)
{
   ...
   va_start(ap, fmt);
   ret = memlog_vappend(UNIFROG_LOG_INFO, "app", fmt, ap);
   va_end(ap);
   return ret;
}
   ...
   const char *fmt, ...)
{
   ...
   va_start(ap, fmt);
   ret = memlog_vappend(level, component, fmt, ap);
   va_end(ap);
   return ret;
}
...
int unifrog_log_sync(const char *fmt, ...)
{
   ...
   memlog_syncing = 1;
   out_len = snprintf(out, sizeof(out),
      "UFLOG type=sync ts_ms=%lu cyc=%08lx seq=%lu level=warn component=storage.sync msg=%.*s%s",
      (unsigned long)unifrog_perf_time_ms(),
      (unsigned long)unifrog_perf_count(),
...
int unifrog_log_reset(void)
{
   ...
   memlog_flushing = 1;
   memlog_activity_set(UNIFROG_ACTIVITY_PHASE_LOG_RESET,
      (uint32_t)memlog_len, 1);
   fd = memlog_open_retry(O_CREAT | O_WRONLY | O_APPEND);
   if (fd < 0) {
      memlog_flushing = 0;
      memlog_last_result = UNIFROG_LOG_ERR_OPEN;
   ...
   marker_len = snprintf(marker, sizeof(marker),
      "UFLOG type=reset ts_ms=%lu cyc=%08lx seq=%lu level=info component=runtime scpu=%u selector=%u caps=0x%08lx event=boot_start\n",
      (unsigned long)unifrog_perf_time_ms(),
...
int unifrog_log_flush(void)
{
   ...
   header_len = snprintf(header, sizeof(header),
      "\nUFLOG type=flush ts_ms=%lu cyc=%08lx seq=%lu level=debug component=log path=%s pending=%lu dropped=%d\n",
      (unsigned long)unifrog_perf_time_ms(),
   ...
   if (dropped)
      write(fd, "UFLOG type=drop level=error component=log reason=memory_buffer_full\n",
         sizeof("UFLOG type=drop level=error component=log reason=memory_buffer_full\n") - 1);
   memlog_activity_set(UNIFROG_ACTIVITY_PHASE_LOG_FLUSH,
      (uint32_t)pending, 3);
