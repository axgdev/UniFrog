...
static int media_play_native_audio_compressed(const char *path)
{
   ...
   if (avformat_open_input(&fmt, path, NULL, NULL) < 0 ||
      ...
      printf("unifrog media auddec open_input failed path=%s\n", path);
      goto out;
   }
   stream = av_find_best_stream(fmt, AVMEDIA_TYPE_AUDIO, -1, -1, NULL, 0);
   if (stream < 0) {
   ...
   int sync_mode, struct media_auddec *auddec)
{
   ...
   for (unsigned i = 0; i < ARRAY_SIZE(variants); i++) {
      ...
      if (cfg.extradata_mode == 1)
         extra_ret = media_send_extra_packet(fd, par->extradata,
            par->extradata_size);
      errno = 0;
      init_ret = extra_ret == 0 ? ioctl(fd, AUDDEC_INIT, &cfg) : -1;
      init_errno = errno;
   ...
   int disable_audio)
{
   ...
   printf("unifrog media native video open fd=%d ret init=%d init_errno=%d start=%d start_errno=%d rect=%d codec=%u av=%d tag=0x%lx %dx%d fps_milli=%u kshm=%lu extra=%d mode=%d\n",
      fd, init_ret, init_errno, start_ret, start_errno, rect_ret,
   ...
   const struct unifrog_media_video_options *options)
{
   ...
   if (avformat_open_input(&fmt, path, NULL, NULL) < 0 ||
       avformat_find_stream_info(fmt, NULL) < 0) {
      printf("unifrog media native open failed path=%s\n", path);
      goto out;
   }
...
out:
   ...
   media_auddec_close(&auddec);
   if (video_fd >= 0) {
      (void)ioctl(video_fd, VIDDEC_FLUSH, 0);
      close(video_fd);
   }
   close_display();
   free(pcm);
