...
#define CORE_MODULE_TIMED_READ_CHUNK (32u * 1024u)
...
static int read_available_fd(int fd, void *dst, size_t bytes,
   size_t *out_done, unsigned timeout_ms, const char *path)
{
   ...
   while (done < bytes) {
      ...
      if (timeout_ms && chunk > CORE_MODULE_TIMED_READ_CHUNK)
         chunk = CORE_MODULE_TIMED_READ_CHUNK;
      ...
      if (timeout_ms && elapsed_ms(start_ms, now_ms) > timeout_ms) {
         printf("unifrog core_module read_timeout path=%s done=%u total=%u elapsed=%u timeout=%u\n",
            path ? path : "", (unsigned)done, (unsigned)bytes,
            elapsed_ms(start_ms, now_ms), timeout_ms);
         unifrog_log_sync("core_module read_timeout path=%s done=%u total=%u elapsed=%u timeout=%u",
            path ? path : "", (unsigned)done, (unsigned)bytes,
            elapsed_ms(start_ms, now_ms), timeout_ms);
         unifrog_platform_sd_debug_dump("core_module_read_timeout");
         if (out_done)
            *out_done = done;
         errno = ETIMEDOUT;
         return -1;
...
static int load_image_payload_fd(int fd, uintptr_t load_addr,
   size_t image_size, unsigned timeout_ms, const char *path)
{
   ...
   if (read_available_fd(fd,
       ...
       payload_size, &payload_read, timeout_ms, path) != 0)
      return -1;
   ...
   unsigned read_timeout_ms)
{
   ...
   unifrog_log_sync("core_module load begin path=%s expected=%s timeout=%u",
      path, expected_id ? expected_id : "", read_timeout_ms);
   ...
   if (read_available_fd(fd, &header, sizeof(header), &header_read, 0,
       path) != 0 || header_read != sizeof(header)) {
   ...
   if (load_image_payload_fd(fd, header.load_addr, image_size,
       read_timeout_ms, path) != 0) {
      printf("unifrog core_module read_image_failed path=%s image=%u timeout=%u errno=%d\n",
         path, (unsigned)image_size, read_timeout_ms, errno);
      goto out_close;
   }
   ...
   unifrog_log_sync("core_module load done id=%s path=%s total=%u read=%u image=%u memory=%u timeout=%u",
      header.core_id, path, elapsed_ms(total_start_ms, entry_done_ms),
      elapsed_ms(validate_done_ms, read_done_ms), (unsigned)image_size,
      (unsigned)memory_size, read_timeout_ms);
   ret = 0;

out_close:
   if (ret != 0)
      unifrog_log_sync("core_module load fail path=%s expected=%s timeout=%u errno=%d",
         path, expected_id ? expected_id : "", read_timeout_ms, errno);
   close(fd);
   return ret;
}
...
{
   return unifrog_core_module_load_file_timeout(path, expected_id, loaded, 0);
}
