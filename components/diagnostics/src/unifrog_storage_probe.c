...
#define FAST_PROBE_SCRATCH_PATH \
   UNIFROG_DIST_ROOT "/storage-fast-probe-scratch.txt"
...
static int fast_probe_write_report(const char *report, size_t report_used)
{
   if (!report || report_used == 0)
      return -1;
   return unifrog_storage_write_atomic(UNIFROG_STORAGE_FAST_PROBE_REPORT,
      NULL, report, report_used, "fast_probe_report", 1, 0);
}
...
static int write_probe_write_report(const char *report, size_t report_used)
{
   if (!report || report_used == 0)
      return -1;
   return unifrog_storage_write_atomic(UNIFROG_STORAGE_WRITE_PROBE_REPORT,
      NULL, report, report_used, "write_probe_report", 1, 0);
}
   ...
   unsigned chunks, int fsync_each, uint32_t *write_ms, uint32_t *fsync_ms)
{
   ...
   for (unsigned i = 0; i < chunks; i++) {
      ...
      unifrog_diagnostic_fill_pattern(buffer, WRITE_PROBE_CHUNK_BYTES, i);
   ...
   void *userdata, char *report, size_t report_size, size_t *report_used)
{
   ...
   for (unsigned i = 0; i < WRITE_PROBE_SMALL_FILES; i++) {
      ...
      unifrog_diagnostic_fill_pattern(buffer, sizeof(buffer), i);
...
{
   struct unifrog_platform_debug_status status;

   unifrog_platform_debug_status(&status);
   printf("unifrog fast_sd_probe fileuart tag=%s pending=%lu suspended=%lu quiet=%lu dirty=%lu\n",
      tag ? tag : "", (unsigned long)status.pending,
      (unsigned long)status.suspended, (unsigned long)status.quiet_ticks,
      (unsigned long)status.dirty_bytes);
}
   ...
   unsigned file_index, const char *phase)
{
   ...
   ret = unifrog_platform_debug_write(text, (size_t)len,
      FAST_PROBE_FILEUART_BURST_LINES);
   err = ret < 0 ? -ret : 0;
   ...
   unsigned max_paths)
{
   static const char *known_paths[] = {
      UNIFROG_ROMS_ROOT "/probes/test20M.md",
      UNIFROG_ROMS_ROOT "/probes/test10M.md",
      UNIFROG_ROMS_ROOT "/probes/test5M.md",
      UNIFROG_ROMS_ROOT "/probes/test1M.md",
      UNIFROG_SD_ROOT "/firmware/unifrog.bin",
      UNIFROG_CORE_ROOT "/quicknes.bin",
      UNIFROG_CORE_ROOT "/pce-fast.bin",
      UNIFROG_CORE_ROOT "/picodrive.bin",
      UNIFROG_BIOS_ROOT "/bisrv.asd",
   };
   FILE *index;
   char line[640];
   unsigned count = 0;
   ...
   index = fopen(UNIFROG_DIST_ROOT "/game-index.txt", "rb");
   if (!index)
      return count;
...
static int fast_probe_read_file(const char *path, struct fast_probe_read *out)
{
   ...
   while (total < FAST_PROBE_READ_LIMIT) {
      ...
      if (got == 0)
         break;
      checksum = unifrog_diagnostic_checksum_update(checksum, buffer,
         (size_t)got);
   ...
   out->bytes = total;
   out->kib_s = unifrog_diagnostic_kib_per_second(total, out->ms);
   ...
   void *userdata, char *summary, size_t summary_size)
{
   ...
   fast_probe_append(report, FAST_PROBE_REPORT_BYTES, &report_used,
      "show=1\n"
      "title=FAST SD PROBE\n"
      "item|OK|Build|mode=%s commit=%s|runtime=%d\n"
      ...
      UNIFROG_SD_MODE, UNIFROG_GIT_COMMIT, runtime_supported,
   ...
   void *userdata, char *summary, size_t summary_size)
{
   ...
   fast_probe_append(report, WRITE_PROBE_REPORT_BYTES, &report_used,
      ...
      "item|OK|Build|mode=%s commit=%s|chunk=%u chunks=%u\n"
      "item|OK|Purpose|separate write, close, fsync|report checkpoints after each phase\n",
      UNIFROG_SD_MODE, UNIFROG_GIT_COMMIT, WRITE_PROBE_CHUNK_BYTES,
      WRITE_PROBE_CHUNKS);
   (void)write_probe_write_report(report, report_used);
   (void)unifrog_log_flush_force();
