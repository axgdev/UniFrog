...
static void fill_rgb565(uint16_t *pixels, unsigned width, unsigned height)
{
   for (unsigned y = 0; y < height; y++) {
      for (unsigned x = 0; x < width; x++) {
         pixels[y * width + x] = unifrog_diagnostic_rgb565_pixel(
            x, y, width, height);
      }
   }
}
...
static void fill_xrgb8888(uint32_t *pixels, unsigned width, unsigned height)
{
   for (unsigned y = 0; y < height; y++) {
      for (unsigned x = 0; x < width; x++) {
         pixels[y * width + x] = unifrog_diagnostic_xrgb8888_pixel(
            x, y, width, height);
      }
   }
}
...
static int open_fb_for_test(struct unifrog_fb *fb, unsigned flags)
{
   int last_errno = 0;

   for (unsigned attempt = 0; attempt < 3u; attempt++) {
      errno = 0;
      if (unifrog_fb_open(fb, flags) == 0) {
         errno = 0;
         return 0;
      }
      last_errno = errno;
      if (last_errno != ENOMEM && last_errno != EAGAIN)
         break;
      unifrog_task_delay_ms(120);
   }
   ...
   enum unifrog_ge_format format, unsigned src_bpp, const char *ppm_path)
{
   ...
   errno = 0;
   if (open_fb_for_test(&fb, fb_flags) != 0)
      goto out;
   ...
   unsigned *best_total_count, const char **best_name)
{
   ...
   if (!bench)
      return -1;
   errno = 0;
   if (open_fb_for_test(&fb, bench->fb_flags) != 0)
      goto out;
...
out:
   ...
   if (bench && (bench->fb_flags & UNIFROG_FB_OPEN_XRGB8888)) {
      ...
      if (open_fb_for_test(&restore, UNIFROG_FB_OPEN_DEFAULT) == 0)
...
int unifrog_display_benchmark_run(char *summary, size_t summary_size)
{
   ...
   /* Return the frontend to its normal low-bandwidth scanout mode. */
   {
      ...
      memset(&fb, 0, sizeof(fb));
      if (open_fb_for_test(&fb, UNIFROG_FB_OPEN_DEFAULT) == 0)
         unifrog_fb_close(&fb);
   }
