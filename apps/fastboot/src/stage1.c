...
#define DEFAULT_ASD_CFG_READ_MAX (FASTBOOT_HANDOFF_PATH_BYTES - 1u)
#define FASTBOOT_PINMUXL_BASE 0xb88004a0u
#define FASTBOOT_PINMUXR_BASE 0xb88004e0u
...
#define FASTBOOT_GPIOR_INPUT_REG 0xb88000f0u
...
#define FASTBOOT_AMP_GB300_PIN 15u
#define FASTBOOT_AMP_SF2000_PIN 7u
#define FASTBOOT_BACKLIGHT_PIN 5u
#define FASTBOOT_GPIO_L27_MASK (1u << 27u)
#define FASTBOOT_GPIO_R07_MASK (1u << 7u)
...
static uint32_t fastboot_hw_l_in;
static uint32_t fastboot_hw_l_out;
static uint32_t fastboot_hw_l_dir;
static uint32_t fastboot_hw_r_in;
static uint32_t fastboot_hw_r_out;
static uint32_t fastboot_hw_r_dir;
static uint32_t fastboot_key_sf2000_and;
static uint32_t fastboot_key_sf2000_or;
static uint32_t fastboot_key_sf2000_last;
static uint32_t fastboot_key_gb300_and;
static uint32_t fastboot_key_gb300_or;
static uint32_t fastboot_key_gb300_last;
static uint32_t fastboot_detected_board;
static uint32_t fastboot_detected_board_marks;
static uint32_t fastboot_detected_board_scores;
...
static uint32_t fastboot_scan_sf2000_b_key(void)
{
	uint32_t sf2000;
	unsigned int i;
...
static uint32_t fastboot_scan_gb300_b_key(void)
{
	uint32_t gb300;
	unsigned int i;
...
static uint32_t fastboot_scan_boot_cancel(void)
{
	uint32_t key;

	if (fastboot_detected_board == FASTBOOT_DEVICE_BOARD_GB300) {
		key = fastboot_scan_gb300_b_key();
		if (key != 0)
			return key;
		return fastboot_scan_sf2000_b_key();
	}

	key = fastboot_scan_sf2000_b_key();
	if (key != 0)
		return key;
	return fastboot_scan_gb300_b_key();
}
...
static unsigned int fastboot_pack4(unsigned int a, unsigned int b,
...
static void fastboot_log_passive_hardware_fingerprint(void)
{
	fastboot_hw_l_in = read_reg32(FASTBOOT_GPIOLCTRL_BASE +
		FASTBOOT_GPIO_INPUT_ST_REG);
	fastboot_hw_l_out = read_reg32(FASTBOOT_GPIOLCTRL_BASE +
		FASTBOOT_GPIO_OUTPUT_VAL_REG);
	fastboot_hw_l_dir = read_reg32(FASTBOOT_GPIOLCTRL_BASE +
		FASTBOOT_GPIO_DIR_REG);
	fastboot_hw_r_in = read_reg32(FASTBOOT_GPIOR_INPUT_REG);
	fastboot_hw_r_out = read_reg32(FASTBOOT_GPIOR_OUTPUT_REG);
	fastboot_hw_r_dir = read_reg32(FASTBOOT_GPIOR_DIR_REG);

	boot_trace_note(FASTBOOT_TRACE_STAGE1_HW_GPIO_L,
		fastboot_hw_l_in, fastboot_hw_l_out, fastboot_hw_l_dir);
	boot_trace_note(FASTBOOT_TRACE_STAGE1_HW_GPIO_R,
		fastboot_hw_r_in, fastboot_hw_r_out, fastboot_hw_r_dir);
	boot_trace_note(FASTBOOT_TRACE_STAGE1_HW_PINMUX,
		fastboot_pack4(
			read_reg8(FASTBOOT_PINMUXL_BASE + FASTBOOT_AMP_GB300_PIN),
			read_reg8(FASTBOOT_PINMUXL_BASE + FASTBOOT_KEY_SF2000_PL1_PIN),
			read_reg8(FASTBOOT_PINMUXL_BASE + FASTBOOT_KEY_SF2000_CLK_PIN),
			read_reg8(FASTBOOT_PINMUXL_BASE + FASTBOOT_KEY_GB300_D1_PIN)),
		fastboot_pack4(
			read_reg8(FASTBOOT_PINMUXL_BASE + FASTBOOT_KEY_GB300_CLK_PIN),
			read_reg8(FASTBOOT_PINMUXL_BASE + FASTBOOT_KEY_GB300_D0_PIN),
			read_reg8(FASTBOOT_PINMUXL_BASE + 28u),
			read_reg8(FASTBOOT_PINMUXL_BASE + 29u)),
		fastboot_pack4(
			read_reg8(FASTBOOT_PINMUXR_BASE + FASTBOOT_BACKLIGHT_PIN),
			read_reg8(FASTBOOT_PINMUXR_BASE + FASTBOOT_AMP_SF2000_PIN),
			0,
			0));
}

static void fastboot_log_key_fingerprint(void)
{
	unsigned int i;

	fastboot_key_sf2000_and = FASTBOOT_SF2000_KEY_MASK;
	fastboot_key_sf2000_or = 0;
	fastboot_key_sf2000_last = 0;
	fastboot_key_gb300_and = FASTBOOT_GB300_KEY_MASK;
	fastboot_key_gb300_or = 0;
	fastboot_key_gb300_last = 0;

	for (i = 0; i < FASTBOOT_BUTTON_SCAN_POLLS; i++) {
		fastboot_key_sf2000_last = fastboot_scan_sf2000_keys() &
			FASTBOOT_SF2000_KEY_MASK;
		fastboot_key_sf2000_and &= fastboot_key_sf2000_last;
		fastboot_key_sf2000_or |= fastboot_key_sf2000_last;
	}

	for (i = 0; i < FASTBOOT_BUTTON_SCAN_POLLS; i++) {
		fastboot_key_gb300_last = fastboot_scan_gb300_keys() &
			FASTBOOT_GB300_KEY_MASK;
		fastboot_key_gb300_and &= fastboot_key_gb300_last;
		fastboot_key_gb300_or |= fastboot_key_gb300_last;
	}

	boot_trace_note(FASTBOOT_TRACE_STAGE1_INPUT_SF2000_RAW,
		fastboot_key_sf2000_and, fastboot_key_sf2000_or,
		fastboot_key_sf2000_last);
	boot_trace_note(FASTBOOT_TRACE_STAGE1_INPUT_GB300_RAW,
		fastboot_key_gb300_and, fastboot_key_gb300_or,
		fastboot_key_gb300_last);
}

static void fastboot_detect_board(void)
{
	unsigned int sf2000_score = 0;
	unsigned int gb300_score = 0;
	uint32_t marks = 0;

	if (fastboot_key_gb300_and == FASTBOOT_GB300_KEY_MASK &&
			fastboot_key_gb300_or == FASTBOOT_GB300_KEY_MASK) {
		sf2000_score += 2u;
		marks |= FASTBOOT_BOARD_MARK_GB300_BUS_SATURATED;
	} else if (fastboot_key_gb300_and == 0 &&
			fastboot_key_gb300_or == 0) {
		gb300_score += 2u;
		marks |= FASTBOOT_BOARD_MARK_GB300_BUS_ZERO;
	} else if (fastboot_key_gb300_or != FASTBOOT_GB300_KEY_MASK) {
		gb300_score += 1u;
		marks |= FASTBOOT_BOARD_MARK_GB300_BUS_ACTIVE;
	}

	if (fastboot_hw_l_in & FASTBOOT_GPIO_L27_MASK) {
		gb300_score++;
		marks |= FASTBOOT_BOARD_MARK_L27_HIGH;
	} else {
		sf2000_score++;
		marks |= FASTBOOT_BOARD_MARK_L27_LOW;
	}

	if (fastboot_hw_r_in & FASTBOOT_GPIO_R07_MASK) {
		sf2000_score++;
		marks |= FASTBOOT_BOARD_MARK_R07_HIGH;
	} else {
		gb300_score++;
		marks |= FASTBOOT_BOARD_MARK_R07_LOW;
	}

	if (sf2000_score >= 2u && sf2000_score >= gb300_score + 2u)
		fastboot_detected_board = FASTBOOT_DEVICE_BOARD_SF2000;
	else if (gb300_score >= 2u && gb300_score >= sf2000_score + 2u)
		fastboot_detected_board = FASTBOOT_DEVICE_BOARD_GB300;
	else
		fastboot_detected_board = FASTBOOT_DEVICE_BOARD_UNKNOWN;

	fastboot_detected_board_marks = marks;
	fastboot_detected_board_scores = (sf2000_score << 16) | gb300_score;
	boot_trace_note(FASTBOOT_TRACE_STAGE1_BOARD_DETECT,
		fastboot_detected_board, fastboot_detected_board_marks,
		fastboot_detected_board_scores);
}
...
static int fastboot_valid_asd_path(const char *path)
{
	unsigned int i;
	...
	for (i = 0; path[i] != '\0'; i++) {
		...
		if (i >= DEFAULT_ASD_CFG_READ_MAX)
			return 0;
		if (c == '\\' || c == ':' || fastboot_ascii_is_space(c))
			return 0;
		if (c == '/') {
			if (component_start)
				return 0;
	...
	return !component_start && fastboot_ascii_ends_asd(path);
}
...
static int read_default_asd_path(char *path, unsigned int path_size)
{
	...
	if (end <= begin || end - begin >= path_size) {
		boot_trace_note(FASTBOOT_TRACE_STAGE1_DEFAULT_RESULT,
	...
	for (i = 0; i < path_len; i++)
		path[i] = cfg[begin + i];
	path[path_len] = '\0';
	if (!fastboot_valid_asd_path(path)) {
		boot_trace_note(FASTBOOT_TRACE_STAGE1_DEFAULT_RESULT,
			(unsigned int)-5, trace_path_hash(path), path_len);
		return -1;
	}
	return 0;
}
...
void stage1_main(void)
{
	...
	fastboot_log_passive_hardware_fingerprint();
	fastboot_log_key_fingerprint();
	fastboot_detect_board();
	fastboot_backlight_off();
	...
	boot_cancel_keys = fastboot_scan_boot_cancel();
	if (boot_cancel_keys != 0) {
		boot_trace_note(FASTBOOT_TRACE_STAGE1_INPUT_OVERRIDE,
			boot_cancel_keys, fastboot_last_sf2000_keys,
			fastboot_last_gb300_keys);
		rom_printf("fastboot: B cancelled default keys=0x%08x sf2000=0x%08x gb300=0x%08x\n",
			(unsigned int)boot_cancel_keys,
