...
for (var i = 0; i < profiles.length; i++) {
  showStatus("Stable storage stress",
    (i + 1) + "/" + profiles.length + " " + profiles[i],
    "Do not power off unless this stops updating");
  var ret = JS2300.system.action("developer:storage_stress:" + profiles[i]);
  JS2300.log("storage stable stress profile=" + profiles[i] + " ret=" + ret);
  if (ret <= 0)
    failures++;
  JS2300.sleep(250);
}
...
if (failures)
  throw new Error("stable storage failures=" + failures);
