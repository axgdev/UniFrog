var FD = (function() {
  ...
  var LOG_DIR = ROOT + "/unifrog_data/logs";
  var STATE = LOG_DIR + "/frontend-driver-state.ini";
  var RESUME_STATE = ROOT + "/unifrog_data/frontend-driver.state";
  var REPORT = LOG_DIR + "/frontend-driver-report.txt";
  ...
  function readText(path) {
    try { return JS2300.fs.readText(path) || ""; } catch (readTextErr) { return ""; }
  }
  function writeText(path, text) {
    JS2300.fs.writeText(path, text);
  }
  function flushLog() {
    try { JS2300.flushLog(); } catch (flushLogErr) {}
  }
  ...
  function notify(text) {
    try {
      ...
      JS2300.video.text(14, 18, "UniFrog storage test", 0xffff);
      JS2300.video.text(14, 52, text, 0xbdf7);
      JS2300.video.text(14, 206, "Do not power off unless this stops updating", 0x7bef);
      JS2300.video.present();
    } catch (notifyVideoErr) {}
    try { JS2300.system.action("toast:" + text); } catch (notifyToastErr) {}
  }
  ...
  function list(path) {
    try { return JS2300.fs.list(path) || []; } catch (listErr) { return []; }
  }
  ...
  function runSuiteWithOptions(suite, tests, options) {
    options = options || {};
    ...
    for (; step < tests.length; step++) {
      ...
      append("PROGRESS suite=" + suite + " step=" + (step + 1) +
        "/" + tests.length + " name=" + tests[step].name);
      try {
        var ret = tests[step].run();
        crashes = 0;
        if (ret !== 0 && options.stopOnFail) {
          append("ABORT suite=" + suite + " step=" + step +
            " name=" + tests[step].name + " reason=test_failed");
          disableState("test_failed", suite, step, tests[step].name, crashes);
          return;
        }
      } catch (suiteErr) {
        append("FAIL " + suite + "." + tests[step].name + " exception=" + suiteErr);
        if (options.stopOnFail) {
          disableState("exception", suite, step, tests[step].name, crashes);
          return;
        }
      }
  ...
  function runSuite(suite, tests) {
    return runSuiteWithOptions(suite, tests, {});
  }
  ...
  return {
    ...
    main: main,
    runSuite: runSuite,
    runSuiteWithOptions: runSuiteWithOptions,
    runCore: runCore,
    readText: readText
  };
})();
