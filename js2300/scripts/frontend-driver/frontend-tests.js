...
FD.frontendTests = [
  ...
  { name: "backlight_sweep", run: function() {
    ...
    for (var i = 0; i < levels.length; i++) {
      ...
      if (ret !== levels[i] || current !== levels[i]) {
        try { JS2300.system.backlight(original); } catch (backlightFailRestoreErr) {}
    ...
    try { JS2300.system.backlight(original); } catch (backlightRestoreErr) {}
  ...
  { name: "av_output_modes", run: function() {
    ...
    var modes = [0, 1];
    var details = [];
    try { original = JS2300.system.avOutput(); } catch (avReadErr) {}
    for (var i = 0; i < modes.length; i++) {
      ...
      if (ret !== modes[i] || current !== modes[i]) {
        try { JS2300.system.avOutput(original); } catch (avFailRestoreErr) {}
    ...
    try { JS2300.system.avOutput(original); } catch (avRestoreErr) {}
